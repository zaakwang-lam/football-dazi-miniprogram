...
Page({
  data: {
    userInfo: null, unread: 2, myTeams: [], currentRole: '',
    hasUser: false, hasCourt: false, needSelectRole: false, needAuth: false,
    showProfilePanel: false, draftNickname: ''
  },
  ...
  async loadUser() {
    ...
    try {
      ...
      if (res.code !== 0 || !res.data) {
        ...
        if (!isProfileComplete(cached || {}) || localRoles.length === 0) wx.redirectTo({ url: '/pages/login/login' });
        return;
      }
      const serverUser = res.data;
      const roles = normalizeRoles(serverUser.roles);
      let currentRole = '';
      if (roles.length) currentRole = (serverUser.role && roles.includes(serverUser.role)) ? serverUser.role : roles[0];
      const localAvatar = cached?.avatarUrl || '';
      const serverAvatar = serverUser.avatarUrl || '';
      const avatarUrl = serverAvatar || (localAvatar && !/^https?:\/\//i.test(localAvatar) ? localAvatar : '') || localAvatar || '';
      const merged = {
        ...cached, ...serverUser, roles, role: currentRole, registered: roles.length > 0,
        nickName: (serverUser.nickname && serverUser.nickname !== '微信用户') ? serverUser.nickname : (cached?.nickName || cached?.nickname || ''),
        nickname: (serverUser.nickname && serverUser.nickname !== '微信用户') ? serverUser.nickname : (cached?.nickname || cached?.nickName || ''),
        avatarUrl, courtInfo: serverUser.court || cached?.courtInfo || null
      };
      wx.setStorageSync('userInfo', merged);
      app.globalData.userInfo = merged;
      this.applyUserState(merged);
      if (!isProfileComplete(merged) || roles.length === 0) {
        wx.redirectTo({ url: '/pages/login/login' });
        return;
      }
      ...
      if (token && (!isProfileComplete(cached || {}) || normalizeRoles(cached?.roles).length === 0)) {
        wx.redirectTo({ url: '/pages/login/login' });
      }
    }
  },
  ...
  applyUserState(userInfo) {
    ...
    const currentRole = roles.length ? ((userInfo?.role && roles.includes(userInfo.role)) ? userInfo.role : roles[0]) : '';
    const normalized = userInfo ? { ...userInfo, roles, role: currentRole, registered: roles.length > 0 } : null;
    this.setData({
      userInfo: normalized, hasUser: roles.includes('user'), hasCourt: roles.includes('court'),
      currentRole, needSelectRole: !!wx.getStorageSync('token') && roles.length === 0,
  ...
  onSwitchRole() {
    ...
    if (newRole === 'user') this.loadMyTeams(); else this.setData({ myTeams: [] });
  },
  ...
  _previewAndUploadAvatar(localPath) {
    ...
    this.setData({ userInfo: previewUser }); wx.setStorageSync('userInfo', previewUser); app.globalData.userInfo = previewUser;
      ...
      wx.getFileSystemManager().readFile({
        ...
        success: async (fileRes) => {
          try {
            ...
            wx.setStorageSync('userInfo', userInfo); app.globalData.userInfo = userInfo;
            this.setData({ userInfo, showProfilePanel: false });
            wx.showToast({ title: '头像已更新', icon: 'success' });
          } catch (e) { wx.showToast({ title: e.message || '头像上传失败', icon: 'none' }); }
      ...
      success: (res) => {
        ...
        wx.getFileSystemManager().readFile({
          ...
          success: async (fileRes) => {
            ...
            try {
              const r = await api.uploadTeamLogo(id, fileRes.data, 'image/jpeg');
              wx.hideLoading();
              if (r.code === 0) { wx.showToast({ title: '队徽已更新', icon: 'success' }); this.loadMyTeams(); }
            } catch (err) {
              wx.hideLoading();
