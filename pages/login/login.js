...
Page({
  ...
  async onShow() {
    ...
    try {
      ...
      if (res && res.code === 0 && res.data) {
        ...
        const roles = normalizeRoles(server.roles);
        user = {
          ...user,
          ...server,
          roles,
          role: roles.length ? ((server.role && roles.includes(server.role)) ? server.role : roles[0]) : '',
          registered: roles.length > 0,
          nickname: server.nickname || user.nickname || '',
          nickName: server.nickname || user.nickName || user.nickname || '',
          avatarUrl: server.avatarUrl || user.avatarUrl || ''
        };
        wx.setStorageSync('userInfo', user);
        app.globalData.userInfo = user;
      } else if (res && (res.code === 401 || res.code === 404)) {
        wx.removeStorageSync('token');
        wx.removeStorageSync('userInfo');
        app.globalData.token = '';
        app.globalData.userInfo = null;
        this.setData({ step: 'login', agreed: false });
        return;
      }
    } catch (e) {
      console.warn('[login] sync profile fail:', e);
    }
  ...
  onNicknameReview(e) {
    this._editingProfile = true;
    const pass = e && e.detail && (e.detail.pass === true || e.detail.pass === 'true');
    const val = e && e.detail && e.detail.value != null ? String(e.detail.value) : '';
    if (pass && val) {
      this.setData({ draftNickname: val });
    }
  ...
  async onConfirmProfile() {
    ...
    if (!avatarUrl) {
      return wx.showToast({ title: '请选择微信头像或从相册选择', icon: 'none' });
    }
